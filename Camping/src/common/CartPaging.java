...
public class CartPaging {
	...
	//총 게시글 수만 입력하는 생성자
	public CartPaging(int totalCount) {
	...
	public CartPaging(int totalCount, int curPage) {
		this.setTotalCount(totalCount);
	...
	public CartPaging(int totalCount, int curPage, int listCount) {
		this.setTotalCount(totalCount);
		this.setCurPage(curPage);
	...
	public CartPaging(int totalCount, int curPage, int listCount, int pageCount) {
		this.setTotalCount(totalCount);
		this.setCurPage(curPage);
		this.setListCount(listCount);
	...
	private void makePaging() {
		...
		if(listCount == 0)	setListCount(5);	//화면에 보여질 게시글 수 기본값 세팅
		
		//총 페이지 수 계산
		totalPage = totalCount / listCount; //  총 페이지 수 = 총 게시글 수 / 한 페이지당 출력될 게시글 수
		if( totalCount % listCount > 0 )	totalPage++; //  총 게시글 수 / 한 페이지당 출력될 게시글 수의 나머지가 0보다 크면 총 페이지수 +1
