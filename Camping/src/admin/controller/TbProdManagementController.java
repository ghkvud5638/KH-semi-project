...
@WebServlet("/admin/prodmanager")
public class TbProdManagementController extends HttpServlet {
	  	...
	  	if(session.getAttribute("loginid") == null) {//로그인 되어있는지 확인
 			resp.sendRedirect("/user/login");
	  		...
	  		if(("admin").equals(grade)) {//등급이 관리자일 경우에만 관리자페이지 들어갈 수 있음
	  			req.getRequestDispatcher("/WEB-INF/views/admin/tbprod/tbprodmanager.jsp").forward(req, resp);
	  			} else {
